...
const Home = ({ images, setImages, userName }) => {
  ...
  async function likedImages() {
    const likesList = await fetch(`${BACKEND_URL}/get/${userName}`, {
  ...
  useEffect(() => {
    async function fetchPosts() {
      const postList = await fetch(`${BACKEND_URL}/get`, {
        headers: { "Access-Control-Allow-Origin": "*" }
      });
  ...
  const changePage = async (page, pageSize) => {
    ...
    setImgList(
      allImgList.current
        .filter((img) => img.prompt.search(ImageSearch) != -1)
        .slice(
          16 * (page - 1),
          16 * (page - 1) + 16 > images ? images : 16 * (page - 1) + 16
        )
    );
  };
  ...
  const filteredSearch = async (e) => {
    ...
    const fileteredImages = allImgList.current.filter(
      (img) => img.prompt.search(e.target.value) != -1
    );
    setImages(fileteredImages.length);
    setImgList(
      fileteredImages.slice(
        0,
        fileteredImages.length < 16 ? fileteredImages.length : 16
      )
    );
    setPageIndex(1);
  };
  ...
  return (
    <div className="bg-[#f9fafe] min-h-screen border border-t-slate-200">
      ...
      <form className="pt-8 w-11/12 mx-auto max-w-7xl">
        ...
        <input
          name="text-area"
          className="border border-slate-400 w-full rounded text-sm p-2 mt-2 focus:outline-none text-slate-500"
          value={ImageSearch}
      ...
      {!imgList ? (
        <img
          src={loader}
          className="w-8 sm:w-10 md:w-14 xl:w-20 mx-auto mt-8"
        />
      ) : (
        <div className="pt-8 w-11/12 mx-auto grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-w-7xl grid-cols-1">
          {imgList.map((img) => (
            <ShowCase
              key={img._id}
              _id={img._id}
