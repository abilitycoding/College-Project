...
function ShowCase({
  ...
  const [picLiked, setPicLiked] = useState(() =>
    likedPhotos != undefined ? likedPhotos.includes(_id) : false
  );
  ...
  const LikeImage = async () => {
    if (userName) {
      ...
      const req = await fetch(`${BACKEND_URL}/${_id}`, {
        method: "PATCH",
        headers: {
        ...
        body: JSON.stringify({
          likes: !picLiked ? likesNum + 1 : likesNum - 1,
          userName: userName
        })
      });
    ...
    <div className="rounded-xl group relative shadow-card hover:shadow-cardhover card m-2">
      ...
      <div className="group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md">
        ...
        <div className="mt-5 flex justify-between items-center gap-2">
          ...
          <div className="flex">
            <p className=" text-white text-sm text-center pr-1.5 mt-0.5">
              {likesNum >= 1000 ? (likesNum / 1000).toFixed(1) + "K" : likesNum}
            </p>
            {likingImage ? (
              <button
                type="button"
                className="outline-none mr-3 bg-transparent border-none"
                disabled
              >
                <img
                  src={picLiked ? liked : like}
                  alt="download"
