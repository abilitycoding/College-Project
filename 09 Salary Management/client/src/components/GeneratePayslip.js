...
function GeneratePayslip(props) {
  ...
  useEffect(() => {
    ...
    if (userId) {
      ...
      UserService.getUserById(currentUser.id)
        .then((response) => {
          const hireDate = response.data.hasOwnProperty("createdAt")
  ...
  const handleSubmit = () => {
    window.open(
      "http://localhost:5000/api/pdf/payslip/" +
        userId +
        "/" +
        selectedYear +
        "-" +
          ...
          <Form onSubmit={handleSubmit}>
            ...
            {selectedYear !== undefined && (
              ...
              >
                ...
                {months.map((month) => (
                  <option key={month} value={monthNames.indexOf(month) + 1}>
                    {month}
                  </option>
                ))}
              </Form.Select>
            ...
            {selectedMonth !== undefined && (
              <Button variant="success" type="submit">
                Generate payslip for {monthNames[selectedMonth - 1]}{" "}
                {String(selectedYear)}
              </Button>
            )}
          </Form>
